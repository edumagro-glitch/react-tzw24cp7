      ...
      <div style={{ minHeight:"100vh",background:"#0a0e14",fontFamily:"'DM Sans',sans-serif",color:"#e8f0fe",maxWidth:"480px",margin:"0 auto" }}>
        ...
        {tab==="entries" && (
          <div style={{ padding:"1.25rem" }}>
            ...
            <div style={{ fontSize:"0.78rem",color:"#6b7a99",marginBottom:"1.25rem",lineHeight:1.5 }}>
              Filtre por horário e veja quais crianças têm atividade a partir daquele momento.
            </div>
            <div style={{ marginBottom:"1rem" }}>
              <label style={{ display:"block",fontSize:"0.7rem",fontWeight:600,color:"#6b7a99",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:"0.5rem" }}>Horário de entrada</label>
              <select value={entryTimeFilter} onChange={e=>setEntryTimeFilter(e.target.value)}
                style={{ width:"100%",background:"#0d1420",border:"1px solid #2a3548",borderRadius:"10px",
                  padding:"0.65rem 0.9rem",color:"#e8f0fe",fontSize:"0.875rem",outline:"none",
            ...
            {DAYS.map(day=>{
              const slots = (therapistSchedules[day]||[]).filter(s=>s.time>=entryTimeFilter);
              const children = [...new Set(slots.map(s=>s.child))].sort();
              ...
              return (
                <div key={day} style={{ marginBottom:"1.25rem" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"0.6rem" }}>
                    ...
                    <span style={{ fontSize:"0.7rem",color:"#6b7a99" }}>{children.length} criança(s)</span>
                    <div style={{ flex:1,height:"1px",background:"#1e2d45" }}/>
                  </div>
                  {children.map(child=>{
                    const childSlots = slots.filter(s=>s.child===child).sort((a,b)=>a.time.localeCompare(b.time));
                    const firstTime = childSlots[0]?.time;
                    return (
                      <div key={child} style={{ display:"flex",alignItems:"center",gap:"0.6rem",padding:"0.6rem 0.85rem",marginBottom:"0.35rem",background:"#0d1420",border:"1px solid #1e2d45",borderRadius:"8px" }}>
                        <span style={{ fontFamily:"'DM Mono',monospace",fontSize:"0.78rem",color:"#34d399",background:"#0a2e1a",borderRadius:"5px",padding:"0.12rem 0.45rem",minWidth:"52px",textAlign:"center",flexShrink:0 }}>{firstTime}</span>
                        <span style={{ fontSize:"0.85rem",color:"#e8f0fe",flex:1 }}>{child}</span>
                        <span style={{ fontSize:"0.72rem",color:"#6b7a99" }}>{childSlots.length} horário(s)</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {DAYS.every(day=>(therapistSchedules[day]||[]).filter(s=>s.time>=entryTimeFilter).length===0) && (
              <div style={{ textAlign:"center",color:"#6b7a99",fontSize:"0.8rem",padding:"2rem 1rem",background:"#0d1420",borderRadius:"12px",border:"1px dashed #2a3548" }}>
                <div style={{ fontSize:"1.5rem",marginBottom:"0.5rem" }}>🗺️</div>
                Nenhuma criança com entrada a partir das {entryTimeFilter}
                <div style={{ fontSize:"0.72rem",marginTop:"0.4rem",color:"#4a5a70" }}>Tente um horário anterior ou importe as agendas na aba 📋</div>
              </div>
            )}
          </div>
        )}
